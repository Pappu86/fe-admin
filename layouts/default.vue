<template>
  <div id="app">
    <v-app>
      ...
      <!-- Go to top -->
      <app-fab />
      <v-snackbar v-if="snackbar" :value="snackbar.show" :timeout="snackbar.timeout" top :color="snackbar.color"
        @input="$store.commit('CLOSE_SNACKBAR')">
        {{ snackbar.message }}
        <template #action="{ attrs }">
          <v-btn dark text icon v-bind="attrs" @click.native="$store.commit('CLOSE_SNACKBAR')">
...
export default {
  ...
  data() {
    return {
      ...
      developer: 'TheFinancialExpress',
      developer_address: 'https://fe.com',
