              ...
              <v-tab-item v-for="(items, category) in settings" :key="category">
                <v-data-table :headers="headers" :items="items" :loading="loading" hide-default-footer
                  ...
                  <template #item.action="{ item }">
                    <v-btn depressed outlined icon fab dark color="primary" small @click.stop="editItem(item)">
          ...
          <v-dialog v-model="dialog" max-width="500px" persistent>
            <v-card>
              ...
              <validation-observer v-slot="{ handleSubmit, invalid, reset }">
                <v-form @submit.prevent="handleSubmit(updateSetting)" @reset.prevent="reset">
                  <v-card-text>
                    <v-text-field v-model="editedItem.key" label="Key" readonly disabled />
                    <b-text v-model="editedItem.label" rules="required" label="Label" required />
                    <template v-if="typeText">
                      <b-text v-model="editedItem.value" label="Value" required />
                    </template>
                    <template v-if="typeTextarea">
                      <b-textarea v-model="editedItem.value" rules="required" label="Value" required />
                    </template>
                    <template v-if="typeBoolean">
                      <b-switch v-model="editedItem.value" color="primary" class="text-capitalize" true-value="true"
                        false-value="false" :label="`${editedItem.value}`" />
                    </template>
                    <template v-if="typeImage">
                      <v-text-field v-model="image" label="Select Image" append-icon="mdi-attachment" readonly
                        @click="$refs.image.click()" />
                      <input ref="image" type="file" style="display: none" accept="image/*"
                        @change="imageFileOnChange" />
                    </template>
                    <template v-if="typeFile">
                      <v-text-field v-model="file" label="Select File" append-icon="mdi-attachment" readonly
                        @click="$refs.file.click()" />
                      <input ref="file" type="file" style="display: none" @change="fileOnChange" />
                    </template>
                  </v-card-text>
                  <v-divider />
                  <action-button-dialog :loading="loading" :error="invalid" @close="closeDialog" />
                </v-form>
              </validation-observer>
            </v-card>
          </v-dialog>
