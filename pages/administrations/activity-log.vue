      ...
      <v-row>
        <v-col>
          <div style="display: flex">
            ...
            <v-spacer />
            <v-btn v-can="`delete activity`" :loading="deleting" color="pink" class="white--text" @click="deleteAll">
              Delete All
            </v-btn>
          </div>
        </v-col>
        ...
        <v-col>
          <v-card>
            <v-card-title class="pa-1">
              <v-text-field v-model="search" solo flat prepend-inner-icon="mdi-magnify" placeholder="Type something"
                hide-details />
              <v-btn icon>
            ...
            <v-card-text class="pa-0">
              <v-data-table v-model="selected" :headers="headers" :items="activity_logs" :options.sync="pagination"
                :server-items-length="total" :loading="loading" disable-sort show-select class="elevation-1">
                <template #item.created_at="{ item }">
                  {{ getActivityDateTime(item.created_at) }}
                </template>
                <template #item.updated_at="{ item }">
                  {{ getActivityDateTime(item.updated_at) }}
                </template>
                <template #item.action="{ item }">
                  <v-btn v-can="`delete activity`" depressed outlined icon fab dark color="pink" small
...
export default {
  ...
  data() {
    return {
      ...
      getActivityDateTime(dateTime) {
        return this.getDayTimeFormat(dateTime);
      }
    };
  },
  watch: {
